...
% Time-resolved EEG cross-decoding of object location across categories. 
% Saves single subject's decoding time resolved RDM.
% Requirement: libsvm toolbox has to be in the path.

% Duration:
% 5 permutations downsampled to 10 ms resolution takes ~37 seconds
% 100 permutations with full 1 ms resulution takes
...
% then subsample those timepoints from data (if subsampled, otherwise will take all)
...
for iperm = 1:permutations
    ...
    sprintf('Permutation #%d out of %d',iperm,permutations)
    
    % bin the data with random assignment of trials to bins
    ...
    binned_data = squeeze(nanmean(binned_data,2)); % averaged bins to get new pseudo-trials
    ...
    % combinations of categories and in all background conditions
...
% save result
save([savepath 's' sprintf('%.2d',sbj) '_Location_Timecourse.mat' ],'RDM','timepoints','timewindow','duration','-v7.3');
